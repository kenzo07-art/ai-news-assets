...
- **フォント指定はテンプレートに書かれている `font-family:'Meiryo UI',Meiryo,'Hiragino Kaku Gothic ProN','Segoe UI',sans-serif` をそのまま写す**。順番も変えない（Windows/Outlookでは Meiryo UI、Macではヒラギノ、という出し分けをこの順番でやっているため）
- `{{CAT_COLOR}}` はカテゴリごとに: 世界=`#2f6fd0` / 日本=`#1f9d63` / 企業活用=`#c8891b` / フィジカル=`#c2553f`
- `{{CAT_NAME}}` は: `世界のAIニュース` / `日本のAIニュース` / `国内企業のAI活用事例` / `フィジカルAIニュース`
- **絵文字を本文・件名・プレーンテキスト版のどこにも使わない**（🌍🇯🇵🏢🤖 のような4バイト文字はGmailの送信経路で `������` に化ける。カテゴリの区別は色付きの縦バーが担っているので絵文字は不要）。`★ ☆ ▶ ■ ▼ ─` のような記号は化けないので使ってよい
- `{{STARS}}` は score に応じて `★★★` / `★★☆` / `★☆☆`
